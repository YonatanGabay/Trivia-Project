#pragma once
#include <WinSock2.h>
#include <vector>
#include <string>
#include <string>
#include <iomanip>
#include <sstream>
#include "RecievedMessage.h"
#include "Question.h"

using namespace std;




class Helper
{
public:
	static int getMessageTypeCode(SOCKET sc);
	static int getMessageTypeCode(string message);
	static char* getPartFromSocket(SOCKET sc, int bytesNum, int flags);
	static int getIntPartFromSocket(SOCKET sc, int bytesNum);
	static string getStringPartFromSocket(SOCKET sc, int bytesNum);
	static void sendData(SOCKET sc, std::string message);
	static string getPaddedNumber(int num, int digits);

	static RecievedMessage* parseMessage(SOCKET sock, string message, bool& exit);
	static void getUserNameAndPassword(string message, string& userName, string& password);
	static void getUserNameAndPasswordAndEmail(string message, string& userName, string& password, string& email);
	static string parseIntTo2Str(int val);
	static string parseIntTo3Str(int val);
	static string parseIntTo4Str(int val);
	static string parseIntTo6Str(int val);
	static string buildQueMessage(Question* que);

private:
	static vector<string> getValuesFromMessage(string message);
	static char* Helper::getPartFromSocket(SOCKET sc, int bytesNum);

};


#ifdef _DEBUG // vs add this define in debug mode
#include <stdio.h>
// Q: why do we need traces ?
// A: traces are a nice and easy way to detect bugs without even debugging
// or to understand what happened in case we miss the bug in the first time
#define TRACE(msg, ...) printf(msg "\n", __VA_ARGS__);
// for convenient reasons we did the traces in stdout
// at general we would do this in the error stream like that
// #define TRACE(msg, ...) fprintf(stderr, msg "\n", __VA_ARGS__);

#else // we want nothing to be printed in release version
#define TRACE(msg, ...) printf(msg "\n", __VA_ARGS__);
#define TRACE(msg, ...) // do nothing
#endif